...
if [ ! -f "${BUILD_DIR}/out/lib/libnil-xit-c-api.so" ]; then
    echo "Error: libnil-xit-c-api.so not found at ${BUILD_DIR}/out/lib/"
    exit 1
fi

cp "${BUILD_DIR}/out/lib/libnil-xit-c-api.so" "${DEST_DIR}/libnil-xit-c-api.so"
echo "✓ Copied libnil-xit-c-api.so to ${DEST_DIR}"
